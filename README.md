...
### 3.1.2 네트워크 소켓 통신
프로젝트 주제에 맞춰 프로그램을 설계하고 제작을 해야했다. 하지만 C언어로 진행되는 low-level network programming에 익숙한 조원이 없었고, 소켓 통신에 대한 개념이 완벽하게 잡혀있지 않아 구현하는데 많은 어려움이 있었다. 많은 예제와 tutorial들, 네트워크 공부를 통해 점차 원하는 방향으로 구현해 나갈 수 있었다.
